...
if __name__=="__main__":
  ...
  for layer_idx in range(prune_mask.shape[0]):
    ...
    for matrix_idx in range(prune_mask.shape[1]):
      print("layer id: %d, matrix id: %d" %(layer_idx, matrix_idx))
      layer = prune_mask[layer_idx,matrix_idx]
      all_ele_ = layer.shape[0] * layer.shape[1]
      one_ele_ = np.sum(layer)
      print(layer.shape[0], layer.shape[1])
      print(all_ele_)
      print(one_ele_)
      all_ele += all_ele_
      one_ele += one_ele_
      if layer_idx == 11 and matrix_idx == 5:
        for row in layer:
          for e in row:
            print(e, end=' ')
          print()
  zero_ele = all_ele - one_ele
  print(zero_ele/all_ele)
  # exit()
