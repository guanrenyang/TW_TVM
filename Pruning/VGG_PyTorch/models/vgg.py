...
# }
models_dir = os.path.expanduser('./ckpt')
model_name = {
...
def conv3x3(in_planes: int, out_planes: int, stride: int = 1, groups: int = 1, dilation: int = 1) -> nn.Conv2d:
    ...
    return Conv2dPruner(in_planes, out_planes, kernel_size=3, stride=stride,
                    padding=dilation, groups=groups, bias=False, dilation=dilation, **args)


def conv1x1_source(in_planes: int, out_planes: int, stride: int = 1) -> nn.Conv2d:
    """1x1 convolution"""
...
def conv1x1(in_planes: int, out_planes: int, stride: int = 1) -> nn.Conv2d:
    """1x1 convolution"""
    return Conv2dPruner(in_planes, out_planes, kernel_size=1, stride=stride,
                     bias=False, **args)
...
def make_layers(cfg, batch_norm=False):
    ...
    for v in cfg:
        if v == 'M':
            ...
            conv2d = Conv2dPruner(in_channels, v, kernel_size=3, padding=1)
            if batch_norm:
