...
def run_sample_decode(infer_model, infer_sess, model_dir, hparams,
                      summary_writer, src_data, tgt_data, pruning_hook):
...
def run_avg_external_eval(infer_model, infer_sess, model_dir, hparams,
                          summary_writer, global_step, pruning_hook):
...
def run_full_eval(model_dir,
  ...
  run_sample_decode(infer_model, infer_sess, model_dir, hparams, summary_writer,
                    sample_src_data, sample_tgt_data, pruning_hook)
  return run_internal_and_external_eval(model_dir, infer_model, infer_sess,
                                        eval_model, eval_sess, hparams,
                                        summary_writer, avg_ckpts,pruning_hook=pruning_hook)
...
def print_weights():
    ...
    for weight in params:
        ...
        tf.logging.warning("\"%s\"," %weight.name)

def train(hparams, scope=None, target_session="", ckpt_dir = None):
  ...
  pruning_train_model = model_helper.create_train_model(model_creator, hparams, scope)
  ...
  #masks_shape =[[1024,2048],[1024,2048],[1024,512],[1536,2048],[1024,2048],[1536,512]]
  masks_value ={}
  # for layer in range(len(pruning_layers)):
  #   masks_value[pruning_layers[layer]] = np.ones(masks_shape[layer])

  pruning_hook = PruningHook(pruning_layers, hparams.pruning_type, True, masks_value)
  pruning_hook.sparsity = hparams.sparsity
  ...
  pruning_train_sess = tf.Session(
      target=target_session, config=config_proto, graph=pruning_train_model.graph)
  pruning_train_sess = _HookedSession(pruning_train_sess, hooks=[pruning_hook])

  with pruning_train_model.graph.as_default():
    loaded_pruning_train_model, global_step = model_helper.create_or_load_model(
        pruning_train_model.model, model_dir, pruning_train_sess, "train", ckpt_dir)
    pruning_hook.is_pruning = True
    pruning_hook.insert_masks()
    pruning_hook.masks_now = [0,1,2,3]
    pruning_hook.begin()

  stats, info, start_train_time = before_train(
      loaded_pruning_train_model, pruning_train_model, pruning_train_sess, global_step, hparams, log_f)
  step_result = loaded_pruning_train_model.train(pruning_train_sess)



  pruning_hook.is_pruning = False
  ...
  # This is the training loop.
    ...
    #[utils.print_out(t.name) for op in tf.get_default_graph().get_operations() for t in op.values()]
    #[utils.print_out(n.values()) for n in tf.get_default_graph().get_operations()]


  pruning_hook.is_pruning = False
  with train_model.graph.as_default():
    loaded_train_model, global_step = model_helper.create_or_load_model(
        train_model.model, model_dir, train_sess, "train", ckpt_dir)
    train_sess.run(train_model.model.global_step.initializer)
    global_step = 0
    loaded_train_model.saver.save(
        get_session(train_sess),
  ...
  # if bleu_socre['test_scores']["bleu"] > 26.02:
  #   global_step = 12000
  ...
  while global_step < num_train_steps:
    ...
    try:
      ...
      run_sample_decode(infer_model, infer_sess, model_dir, hparams,
                        summary_writer, sample_src_data, sample_tgt_data, pruning_hook)
      run_external_eval(infer_model, infer_sess, model_dir, hparams,
                        summary_writer, pruning_hook=pruning_hook)
    ...
    if global_step - last_eval_step >= steps_per_eval:
      ...
      run_sample_decode(infer_model, infer_sess,
                        ...
                        sample_tgt_data, pruning_hook)
      run_internal_eval(
          eval_model, eval_sess, model_dir, hparams, summary_writer, pruning_hook=pruning_hook)
    ...
    if global_step - last_external_eval_step >= steps_per_external_eval:
      ...
      run_sample_decode(infer_model, infer_sess,
                        model_dir, hparams, summary_writer, sample_src_data,
                        sample_tgt_data, pruning_hook)
      _, test_scores, _ =run_external_eval(
          infer_model, infer_sess, model_dir,
          hparams, summary_writer,pruning_hook=pruning_hook)
      # if test_scores["bleu"] > 26.02:
      #   break
  ...
  (result_summary, _, final_eval_metrics) = (
      run_full_eval(
          ...
          summary_writer, sample_src_data, sample_tgt_data, avg_ckpts,pruning_hook=pruning_hook))
  ...
  for metric in hparams.metrics:
    ...
    result_summary, best_global_step, _ = run_full_eval(
        ...
        summary_writer, sample_src_data, sample_tgt_data, pruning_hook=pruning_hook)
    ...
    if avg_ckpts:
      ...
      result_summary, best_global_step, _ = run_full_eval(
          ...
          hparams, summary_writer, sample_src_data, sample_tgt_data,pruning_hook=pruning_hook)
      print_step_info("# Averaged Best %s, " % metric, best_global_step, info,
                      result_summary, log_f)
      summary_writer.close()
  

  with open(os.path.join(out_dir, "masks.pkl"), "wb") as file:
