...
class PruningHook(tf.train.SessionRunHook):
    ...
    def print_weights(self):
        for weight in tf.global_variables():
        ...
        if not self.is_pruning:
            ...
            for weight in tf.trainable_variables():
                ...
                if name in self.masks:
                    read_op = next(
                        op for op in weight.op.outputs[0].consumers() if op.type == "ReadVariableOp"
                    )
    ...
    def prune(self, masks_name, masked_weights, mask_values, accumulated_scores, is_masked_weight, masks_now):
        ...
        for layer in scores:
            new_accumulated_scores.append(abs(layer))
        # new_accumulated_scores = abs(scores)#prune_algo.img2col_forward(scores)
